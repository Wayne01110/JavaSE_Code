...
public class CopyFolderDemo {
    public static void main(String[] args) throws IOException {
        ...
        String rcFolderName = srcFolder.getName();

        //创建目的地目录File对象，路径名是模块名+itcast组成（myCharStream\\itcast）
        File destFolder = new File("myCharStream", rcFolderName);
        ...
        for (File srcFile : listFiles) {
            ...
            String srcFileNamr = srcFile.getName();

            //创建目的地文件File对象，路径名是目的地目录+mn.jpg组成（myCharStream\\itcast\\mn.jpg）
            File destFile = new File(destFolder, srcFileNamr);
